...
const isValidThaiId = (s = "") =>
  /^\d-\d{4}-\d{5}-\d{2}-\d$/.test(String(s).trim());
...
/** ✅ merge รายชื่อเก่า + ใหม่ โดยยึด id */
const mergeUsers = (prev = [], fresh = []) => {
  const map = new Map();
  prev.forEach((u) => {
    map.set(u.id, u);
  });
  ...
  // จะไม่ sort ก็ได้ แต่เดี๋ยวตารางจะนิ่งกว่า
  ...
  return (
    <div className="inline-flex rounded-xl overflow-hidden ring-1 ring-indigo-200">
      ...
      <button
        ...
        className={`px-3 py-1.5 text-sm border-l border-indigo-100 transition ${
          v === "admin"
            ? "bg-gradient-to-r from-indigo-600 to-violet-600 text-white"
            : "bg-white text-gray-700 hover:bg-indigo-50"
        }`}
...
const Toggle = ({
  ...
  return (
    <button
      ...
      className={`inline-flex items-center gap-2 ${sizes.text} select-none ${
    ...
    >
      <span
        ...
        } ring-1 ${isOn ? "ring-emerald-400" : "ring-gray-300"}`}
      >
        <span
          className={`absolute top-1/2 -translate-y-1/2 left-1 rounded-full bg-white shadow
            transition-transform duration-200 ${sizes.knob} ${
...
// ดึงทุกกี่วินาทีให้ admin sync กับ backend
const AUTO_SYNC_MS = 10000; // 10 วิ
...
const AdminUsers = () => {
  ...
  const [createBanReason, setCreateBanReason] = useState("");
  ...
  // --------- Load ALL users ----------
  const loadAll = async ({ merge = false } = {}) => {
    if (!token) return;
    try {
      ...
      if (merge) {
        setAllRows((prev) => mergeUsers(prev, fresh));
      } else {
        setAllRows(fresh);
        setPage(1);
      }
  ...
  // ✅ auto-sync แบบเงียบๆ ทุก 10 วิ
  useEffect(() => {
    if (!token) return;
    const id = setInterval(async () => {
      if (syncing) return;
      setSyncing(true);
      await loadAll({ merge: true });
      setSyncing(false);
    }, AUTO_SYNC_MS);
    return () => clearInterval(id);
  }, [token, syncing]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const onChangePageSize = (e) => {
    ...
    const newPage = Math.min(newTotalPages, Math.floor(startIndex / v) + 1);
    setPageSize(v);
    setPage(newPage);
  };
  ...
  const validateEdit = () => {
    ...
    const emailOkE = !editUser.email || isValidEmail(editUser.email);

    setEditErrors({
      id_card: idOkE ? "" : "รูปแบบไม่ถูกต้อง (เช่น 1-2345-67890-12-3)",
      phone: phOkE ? "" : "ต้องเป็นตัวเลข 10 หลัก",
      email: emailOkE ? "" : "อีเมลไม่ถูกต้อง",
    });
    if (!idOkE || !phOkE || !emailOkE)
      showToast("กรอกข้อมูลไม่ถูกต้อง กรุณาตรวจสอบ", "error");
    return idOkE && phOkE && emailOkE;
  };
  ...
  const saveEdit = async () => {
    try {
      ...
      const turningOff = !!editUser._origEnabled && !editUser.enabled;
  ...
  const savePwd = async () => {
    if (!newPwd || newPwd.length < 6)
      return showToast("รหัสผ่านต้องอย่างน้อย 6 ตัว", "error");
    try {
      ...
      console.error(e);
      showToast(
        e?.response?.data?.message || "อัปเดตรหัสผ่านไม่สำเร็จ",
        "error",
        3500
      );
    }
  ...
  const actuallyToggle = async () => {
    ...
    try {
      ...
      console.error(e);
      showToast(
        e?.response?.data?.message || `สลับสถานะไม่สำเร็จ`,
        "error",
        3500
      );
    } finally {
  ...
  const submitBan = async () => {
    ...
    try {
      if (pendingEdit && pendingEdit.userId === user.id) {
        ...
        setEditOpen(false);
        setEditUser(null);
        showToast(`บันทึกและปิดใช้งานผู้ใช้แล้ว: ${user.email}`);
      } else {
        await adminUpdateUser(token, user.id, {
          enabled: false,
          banReason: reason.trim(),
        ...
        showToast(`ปิดใช้งานและระบุเหตุผลแล้ว: ${user.email}`);
      }
      setBanModal({
        open: false,
        user: null,
  ...
  const openCreate = () => {
    ...
    setCreateErrors({ id_card: "", phone: "", email: "" });
    setCreateBanReason("");
    setCreateOpen(true);
  };
  ...
  const saveCreate = async () => {
    if (!createForm.email) {
      setCreateErrors((er) => ({ ...er, email: "กรอกอีเมล" }));
      showToast("กรอกอีเมล", "error");
      return;
    }
    if (!isValidEmail(createForm.email)) {
      setCreateErrors((er) => ({ ...er, email: "อีเมลไม่ถูกต้อง" }));
      showToast("อีเมลไม่ถูกต้อง", "error");
      return;
    }
    if (!createForm.password) {
      showToast("กรอก password", "error");
      return;
    }
    ...
    try {
      ...
      console.error(e);
      showToast(
        e?.response?.data?.message || "สร้างผู้ใช้ไม่สำเร็จ",
        "error",
        3500
      );
    }
    ...
    <div className="max-w-7xl mx-auto p-6 text-gray-800" ref={topRef}>
                ...
                return (
                  <tr key={u.id} className="border-t border-indigo-50">
                    ...
                    <td className="px-4 py-3">
                      ...
                      {u.id_card && (
                        <div className="text-xs text-gray-500">
                          ID: {formatThaiId(u.id_card)}
                        </div>
                      )}
                    </td>
                    <td className="px-4 py-3">
                      <span
                        className={`px-2.5 py-1 rounded-full text-xs ring-1 ${
                          u.role === "admin"
      ...
      >
        <div className="text-sm text-gray-600">
          แสดง {total === 0 ? 0 : startIndex + 1}-{endIndex} จาก {total} รายการ • หน้า{" "}
          <b>{page}</b> / {totalPages}
        </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setCreateForm((f) => ({
                      ...f,
                      phone: formatPhone10(e.target.value),
                    }))
                  }
                  onBlur={() =>
                    setCreateErrors((er) => ({
                      ...er,
                      phone:
                        createForm.phone && !isValidPhone10(createForm.phone)
              ...
              <div className="md:col-span-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setCreateForm((f) => ({
                      ...f,
                      id_card: formatThaiId(e.target.value),
                    }))
                  }
                  onBlur={() =>
                    setCreateErrors((er) => ({
                      ...
                      id_card:
                        createForm.id_card &&
                        !isValidThaiId(createForm.id_card)
              ...
              {!createForm.enabled && (
                <div className="md:col-span-2">
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      setCreateBanReason(e.target.value.slice(0, 300))
                    }
                    rows={3}
                ...
                <input
                  ...
                  onBlur={() =>
                    setEditErrors((er) => ({
                      ...er,
                      phone:
                        editUser.phone && !isValidPhone10(editUser.phone)
                          ? "ต้องเป็นตัวเลข 10 หลัก"
                          : "",
                    }))
                  }
                ...
                <input
                  ...
                  onBlur={() =>
                    setEditErrors((er) => ({
                      ...
                      id_card:
                        editUser.id_card &&
                        !isValidThaiId(editUser.id_card)
                          ? "รูปแบบไม่ถูกต้อง (เช่น 1-2345-67890-12-3)"
                          : "",
                    }))
                  }
          ...
          <div className="w-full max-w-lg rounded-2xl bg-white shadow-xl">
            ...
            <div className="p-5 space-y-3">
              <div className="text-sm text-gray-700">
                ผู้ใช้:{" "}
                <span className="font-medium">{banModal.user?.email}</span>
              </div>
              ...
              <textarea
                ...
                onChange={(e) =>
                  setBanModal((s) => ({
                    ...s,
                    reason: e.target.value.slice(0, 300),
                  }))
                }
                rows={4}
                className="w-full rounded-lg border border-rose-200 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-rose-300"
