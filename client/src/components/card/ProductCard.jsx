...
const ProductCard = ({ item }) => {
  ...
  // รวมสต็อกจากทุก variant เพื่อโชว์ป้าย "สินค้าหมด"
  ...
  return (
    <div
      ...
      className="
        group w-full max-w-sm h-full cursor-pointer
    ...
    >
      ...
      <div className="relative">
        ...
        {/* ป้ายราคา (pill) */}
        <div className="
          absolute left-3 top-3 rounded-full bg-gray-800 text-white
          px-3 py-1 text-sm font-semibold shadow
        ">
        ...
        {totalStock <= 0 && (
          <div className="
            absolute right-3 top-3 rounded-full bg-red-600/90 text-white
            px-3 py-1 text-xs font-medium shadow
          ">
            สินค้าหมด
          </div>
        )}
      ...
      <div className="p-4 flex-1 flex flex-col">
        ...
        {/* แถบดูรายละเอียด (เส้นแบ่ง + hover) */}
        {/* <div className="mt-4 border-t border-gray-100">
          <div className="
            flex items-center justify-between pt-3 text-sm font-medium
            text-gray-700
          ">
...
//       disabled={disabled}
//       className={[
//         "h-9 px-3 rounded-full text-sm border transition-all",
//         "focus:outline-none focus:ring-2 focus:ring-black/10",
//         disabled
//           ? "bg-gray-100 text-gray-400 border-gray-200 cursor-not-allowed"
//           : active
//             ? "bg-black text-white border-black shadow-[0_1px_0_0_rgba(0,0,0,0.25)]"
...
//     variants.find((v) => (v.quantity ?? 0) > 0) ?? (variants[0] ?? null);

//   const sizeOptions = useMemo(() => {
//     const map = new Map();
//     (item.variants || []).forEach((v) => v.size && map.set(v.size.id, v.size.name));
...
//       .map((v) => (v.generation ? { id: v.generation.id, name: v.generation.name } : { id: null, name: "ไม่มีรุ่น" }));
//     const uniq = new Map();
//     list.forEach((g) => uniq.set(g.id, g.name));
//     return Array.from(uniq, ([id, name]) => ({ id, name }));
//   }, [item?.variants, sizeId]);

//   // Auto select ครั้งแรก/เมื่อเปลี่ยน
//   useEffect(() => {
//     if (!item?.variants?.length || sizeId) return;
...
//   useEffect(() => {
//     if (!sizeId) return;
//     const candidates = (item.variants || []).filter((v) => v.sizeId === Number(sizeId));
//     if (!candidates.length) return setGenerationId("");
//     const currentGenId = generationId === "" ? null : Number(generationId);
//     const stillValid = candidates.some((v) => (v.generationId ?? null) === currentGenId);
//     if (!stillValid) {
//       const picked = pickFirstAvailableVariant(candidates);
//       setGenerationId(picked?.generationId ? String(picked.generationId) : "");
//     }
//   }, [sizeId, item?.variants]);

//   useEffect(() => {
//     if (!sizeId) return setGenerationId("");
...
//       toast.success("บันทึกตะกร้าสำเร็จ");
//     } catch (e) {
//       console.error(e);
...
//   const plusDisabled =
//     !selectedVariant || saving || (selectedStock != null && count >= selectedStock);
...
//     >
//       {/* ภาพสินค้า */}
//       <div onClick={() => navigate(`/product/${item.id}`)} className="relative cursor-pointer">
//         <div className="aspect-[4/3] w-full bg-gray-50 flex items-center justify-center">
//           {item.images?.length ? (
//             <img
//               src={item.images[0].url}
...
//         {/* ป้ายราคา (pill โทนดำ) */}
//         <div className="absolute left-3 top-3 rounded-full bg-black/90 text-white px-3 py-1 text-sm font-semibold shadow">
//           ฿{formatPrice(item.price)}
//         </div>

//         {/* หมดสต็อก */}
//         {selectedVariant && (selectedStock ?? 0) <= 0 && (
//           <div className="absolute right-3 top-3 rounded-full bg-red-600/90 text-white px-3 py-1 text-xs font-medium shadow">
//             สินค้าหมด
//           </div>
//         )}
//       </div>
...
//       <div className="p-4 flex-1 flex flex-col">
//         <div className="space-y-1">
//           <p className="text-base font-semibold text-gray-900 line-clamp-1">{item.title}</p>
...
//         {/* เลือกไซซ์ */}
//         <div className="mt-3">
//           <div className="text-xs text-gray-900 mb-2">ไซซ์</div>
...
//         {sizeId && (
//           <div className="mt-3">
//             <div className="text-xs text-gray-900 mb-2">รุ่น</div>
//             <div className="flex flex-wrap gap-2">
//               {generationOptions.map((g) => (
//                 <Chip
//                   key={String(g.id)}
//                   active={String(g.id ?? "") === String(generationId)}
//                   onClick={() => setGenerationId(g.id === null ? "" : String(g.id))}
//                 >
//                   {g.name}
//                 </Chip>
//               ))}
...
//         {/* จำนวน + สต็อก (แคปซูลสอดธีม) */}
//         <div className="mt-2">
//           <div className="text-xs text-gray-900 mb-2">จำนวน</div>
//           <div className="mt-2 flex items-center justify-between">


//             <div
//               className="
//               inline-flex items-center h-9 rounded-full border border-gray-300
...
//                 title={minusDisabled ? "น้อยสุด 1" : "ลดจำนวน"}
//               >
//                 <Minus size={16} />
//               </button>
...
//                 min="1"
//                 className="
//                 w-14 h-9 text-center outline-none border-0
...
//                 className="w-9 h-9 flex items-center justify-center disabled:opacity-40"
//                 onClick={() =>
//                   setCount((c) => Math.min(selectedStock ?? Number.MAX_SAFE_INTEGER, c + 1))
//                 }
